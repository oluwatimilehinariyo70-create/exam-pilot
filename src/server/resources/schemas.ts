...
export const venueInput = z.object({ name: z.string().trim().min(2).max(160), code: z.string().trim().min(2).max(30).regex(/^[A-Za-z0-9][A-Za-z0-9-]*$/), location: z.string().trim().max(160).optional().or(z.literal("")), capacity: z.coerce.number().int().min(1).max(100000), active: z.boolean().default(true) });
