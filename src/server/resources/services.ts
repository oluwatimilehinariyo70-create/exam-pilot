...
export async function createVenue(input: VenueInput, actorId: string) { try { return await prisma.$transaction(async (db) => { const venue = await db.venue.create({ data: { ...input, examCapacity: input.examCapacity ?? input.capacity, location: input.location || null } }); await audit(db, actorId, "VENUE_CREATED", "Venue", venue.id, { code: venue.code, capacity: venue.capacity, examCapacity: venue.examCapacity }); return venue; }); } catch (error) { if (error instanceof Error && "code" in error && error.code === "P2002") throw new AcademicError("VENUE_CODE_EXISTS", "A venue with this code already exists.", {}, 409); throw error; } }
export async function updateVenue(id: string, input: VenueInput, actorId: string) { try { return await prisma.$transaction(async (db) => { const venue = await db.venue.update({ where: { id }, data: { ...input, examCapacity: input.examCapacity ?? input.capacity, location: input.location || null } }); await audit(db, actorId, "VENUE_UPDATED", "Venue", id, { code: venue.code, active: venue.active, examCapacity: venue.examCapacity }); return venue; }); } catch (error) { if (error instanceof Error && "code" in error && error.code === "P2002") throw new AcademicError("VENUE_CODE_EXISTS", "A venue with this code already exists.", {}, 409); throw error; } }
