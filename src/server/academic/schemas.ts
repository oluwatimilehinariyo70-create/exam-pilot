...
export const examPeriodInput = z.object({ sessionId: id, semesterId: id, name: z.string().trim().min(3).max(160), startDate: dateOnly, endDate: dateOnly, active: z.boolean().optional().default(true) }).refine((value) => value.endDate >= value.startDate, { message: "End date must be on or after start date.", path: ["endDate"] });
