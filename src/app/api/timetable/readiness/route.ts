...
export async function GET(request: Request) {
  try { await requireAcademicRead(); const parsed = generationRequest.pick({ academicSessionId: true, semesterId: true, examPeriodId: true }).safeParse(Object.fromEntries(new URL(request.url).searchParams)); if (!parsed.success) return jsonValidationError("Provide a valid academic session, semester, and examination period.", parsed.error.flatten()); return jsonSuccess(await getGenerationReadiness(parsed.data)); } catch (error) { return jsonError(error); }
}
